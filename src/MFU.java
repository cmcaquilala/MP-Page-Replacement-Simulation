...
public class MFU extends BaseReplacement {
	...
	private Page getMostUsed() {
		ArrayList<Page> arr = new ArrayList<Page>();
		
		// get the most frequency page(s)
		Page most = frames.get(0);
		arr.add(most);
		for(int i = 0; i < frames.size(); i++) {
			if(this.getFreq(frames.get(i)) > this.getFreq(most)) {
				arr.clear();
				most= frames.get(i);
				arr.add(most);
			} else if (this.getFreq(frames.get(i)) == this.getFreq(most)) {
